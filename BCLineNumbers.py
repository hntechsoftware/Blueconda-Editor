# This is a modified version of TkLineNums library
# https://pypi.org/project/tklinenums/
...
class TkLineNumbers(Canvas):
    ...
    def redraw(self, *_, font=None) -> None:
        """Redraws the widget"""

        # Resize the widget and set colors (unchanged)
        self.resize()
        self.set_colors()

        # Delete old line numbers (unchanged)
        self.delete("all")

        # Get first and last line numbers (unchanged)
        first_line = int(self.textwidget.index("@0,0").split(".")[0])
        last_line = int(
            self.textwidget.index(f"@0,{self.textwidget.winfo_height()}").split(".")[0]
        )

        # Draw the line numbers, incorporating font configuration
        for lineno in range(first_line, last_line + 1):
            # Check if line is elided (unchanged)
            tags = self.textwidget.tag_names(f"{lineno}.0")
            elide_values = (self.textwidget.tag_cget(tag, "elide") for tag in tags)
            line_elided = any(getboolean(v or "false") for v in elide_values)

            # If the line is not visible, skip it (unchanged)
            dlineinfo = self.textwidget.dlineinfo(f"{lineno}.0")
            if dlineinfo is None or line_elided:
                continue

            # Create the line number, using provided font or default
            self.create_text(
                0
    ...
    def set_justify(self, justify: str) -> None:
        """Sets the justification of the line numbers.

        Args:
            justify: The desired justification ("left", "right", or "center").
        """
        if justify not in ("left", "right", "center"):
            raise ValueError("Invalid justification value. Must be 'left', 'right', or 'center'.")
        self.justify = justify
        self.redraw()
...
(a case not covered in mouse_off_screen_scroll) -- Internal use only"""
    ...
    def set_colors(self, _: Event | None = None) -> None: # I don't know why error is raised here but I'm not using it
...
if __name__ == "__main__":
    ...
    text.bind("<<Modified>>", lambda _: linenums.redraw(font=("Arial", 12)))
